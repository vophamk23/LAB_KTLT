...
{
    // Tạo một mảng mới với kích thước lớn hơn 1 so với mảng cũ
    string *newAddendums = new string[obj.numAddendums + 1];

    // Sao chép các phụ lục cũ sang mảng mới
    for (int i = 0; i < obj.numAddendums; i++)
    {
        newAddendums[i] = obj.addendums[i];
    }

    // Thêm phụ lục mới vào vị trí cuối cùng của mảng mới
    newAddendums[obj.numAddendums] = addendum;

    // Giải phóng mảng cũ nếu nó tồn tại
    if (obj.addendums != nullptr)
    {
        delete[] obj.addendums;
    }

    // Cập nhật con trỏ addendums để trỏ đến mảng mới
    obj.addendums = newAddendums;

    // Tăng số lượng phụ lục lên 1
    obj.numAddendums++;
}